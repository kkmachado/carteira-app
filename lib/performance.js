...
/* IBOV: pontos de fechamento — variação relativa ao primeiro fechamento do range. */
export function ibovSeries(rows, fromISO, toISO) {
  const inRange = rows.filter((r) => r.date >= fromISO && r.date <= toISO);
  if (!inRange.length) return [];
  const base = inRange[0].value;
  return inRange.map((r) => ({ date: r.date, acc: r.value / base - 1 }));
}
