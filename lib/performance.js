...
/* Aporte/retirada entre dois dias. Com ativos: aporte de ativo novo entra pelo
   ...
   aporte em ativo existente entra pelo Δoriginal.

   Resgate PARCIAL sai pelo bruto pro-rata, não pelo Δoriginal: a Pluggy abate do
   amountOriginal só a fatia de principal, mas o dinheiro que deixa a carteira leva
   junto os juros da fatia resgatada. Contado pelo principal, essa diferença vira
   queda de saldo sem contrapartida — ou seja, "prejuízo" no dia do saque. É a mesma
   regra do resgate total generalizada: com Δoriginal = −original, a fórmula devolve
   −gross. Assume resgate proporcional (verdade em CDB/LCA/LCI de balcão); o valor
   exato só sairia das movimentações do ativo, e a diferença é de centavos.

   Sem ativos: fallback Δtotal_original (spec) — sujeito a artefato em resgate. */
export function flowBetween(prev, curr) {
  if (prev.assets && curr.assets) {
    ...
    for (const c of curr.assets) {
      ...
      const delta = c.original - p.original;
      flow += delta < 0 && p.original > 0 ? delta * (p.gross / p.original) : delta;
    }
...
/* Data inicial do período em relação a `refISO` (hoje). "mtd" = 1º do mês,
   "ytd" = 1º de janeiro, "Nm" = N meses atrás, "max" = null (todo o histórico).
   Overflow de fim de mês segue o comportamento do Date (31/mai − 3m ⇒ 2/mar ou
   3/mar), suficiente aqui. */
export function periodStartISO(period, refISO) {
  if (!period || period === "max") return null;
  if (period === "mtd") return `${refISO.slice(0, 7)}-01`;
    ...
    out.set(
      ...
      parsed.map(({ date, active }) => {
        ...
        return {
          ...
          taxes: assets.reduce((s, a) => s + (a.taxes || 0), 0),
...
/* Converte uma linha da tabela `snapshots` (payload = JSON da Pluggy) num dia.
   Considera só status ACTIVE; taxes = IR provisionado (campo `taxes` da Pluggy —
   `taxes2` é tributo financeiro, não IR, e fica fora). */
export function snapshotToDay(row) {
  ...
  return {
    ...
    taxes: active.reduce((s, a) => s + (a.taxes || 0), 0),
