...
/* Devolve o range pedido MAIS a última linha anterior a ele: o IBOV precisa desse
   fechamento como base (o range costuma abrir num dia sem pregão). As séries de
   taxa ignoram linhas com date <= from, então a linha extra não as afeta. */
export function getBenchmarks(db, fromISO) {
  ...
  for (const series of ALL_SERIES) {
    out[series] = db
      .prepare(
        `SELECT date, value FROM benchmarks
          WHERE series = ?
            AND date >= COALESCE((SELECT MAX(date) FROM benchmarks WHERE series = ? AND date <= ?), ?)
          ORDER BY date ASC`
      )
      .all(series, series, from, from);
  }
...
/* Cache defasado: última atualização em outro dia (horário de SP), ou há mais de
   RETRY_MS. A segunda condição existe porque o SGS publica as séries do dia em
   horários diferentes — a Selic sai à tarde, depois do cron das 12:00 — e sem ela
   o dado do dia só entraria no cache 24h depois. A janela de horas mantém a regra
   de nunca bater no SGS a cada load do frontend. */
const RETRY_MS = 3 * 60 * 60 * 1000;
...
export function isStale(db) {
  ...
  if (updatedSP < todaySP) return true;
  return Date.now() - Date.parse(meta.u) > RETRY_MS;
}
